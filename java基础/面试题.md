...
**引用类型：**
- 如String等，本质是对象的引用，JVM栈中存地址，对象在堆中
...
**面试延伸：**
- Integer缓存：-128到127有缓存机制
- 自动拆装箱可能触发NullPointerException
- 集合类只能存引用类型，需装箱
...
4. 如果不为空，遍历链表/红黑树
   -  key相同：替换value
   -  key不同：插入尾部（JDK1.7头插法，1.8尾插法）
5. 链表长度>8且数组长度>64，转换为红黑树
...
| 性能 | 高 | 低 |
| Null支持 | key/value都可以为null | key/value都不可以为null |
| 初始容量 | 16 | 11 |
| 扩容机制 | 2倍 | 2倍+1 |
