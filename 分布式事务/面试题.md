...
```
Phase 1: 准备阶段 - Coordinator → Participant：Can you commit?
Phase 2: 提交阶段 - Coordinator → Participant：COMMIT/ROLLBACK
```
...
**改进：** 减少阻塞时间，引入超时机制，但仍有数据不一致风险

**3. TCC（Try-Confirm-Cancel）**

```
Try：预留资源 → Confirm：确认执行 → Cancel：取消预留
```
